    ...
    if (startArr.length !== 0) {
        for (let i = 0; i < startArr.length; i++) {
            if (startArr[i]) {
                gainbody = startArr[i];
                $.index = i + 1;
                if(gainbody.substr(0,10)==='zqkd_param'){
let abody=gainbody.replace('zqkd_param','p');
let bbody = $.getdata('youth_start').replace(gainbody, abody);
$.setdata(bbody, 'youth_start');
}
                //if(i>=34){continue;}
    ...
    if (lookArr.length !== 0) {
        for (let k = 0; k < lookArr.length; k++) {
            if (lookArr[k]) {
                lookbody = lookArr[k];
                $.index = k + 1;
if(lookbody.substr(0,10)==='zqkd_param'){
let sbody=lookbody.replace('zqkd_param','p');
let tbody = $.getdata('youth_look').replace(lookbody, sbody);
$.setdata(tbody, 'youth_look');
}
